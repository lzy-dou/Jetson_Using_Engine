...
INFER_SIZE = 32*10       # 推理输入尺寸（320更快，640更准，支持320/480/640/960等）
...
def draw_masks(image, masks, class_ids, alpha=0.5, mask_only=False):
    """绘制分割mask。mask_only=True时只显示彩色掩膜(黑底)，否则叠加在原图上。"""
    if mask_only:
...
def main():
    ...
    for i, img_path in enumerate(image_paths, 1):
        ...
        if r.masks is not None and len(r.masks) > 0:
            ...
            annotated = draw_masks(image, resized_masks, class_ids, mask_only=MASK_ONLY)
